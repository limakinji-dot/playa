...
const CFG = {
  ...
  rpcTimeout:   30000,   // timeout RPC per request (ms)
  rpcRetries:   3,       // jumlah retry RPC sebelum skip
};
...
function createProvider(proxyUrl) {
  ...
  // Wrap send() dengan retry logic untuk 504/timeout
  ...
  baseProvider.send = async function(method, params) {
    ...
    for (let attempt = 0; attempt < CFG.rpcRetries; attempt++) {
      try {
        ...
        if (isRetryable && attempt < CFG.rpcRetries - 1) {
